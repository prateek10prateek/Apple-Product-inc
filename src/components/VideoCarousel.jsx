...
const VideoCarousel = () => {
  ...
  useGSAP(() => {
    gsap.to(sliderRef.current, {
      xPercent: -100 * videoId, // Adjust horizontal slide based on videoId
      duration: 0.25,
      ease: 'power2.inOut',
    });
  ...
  useEffect(() => {
    ...
    console.log(videoRef.current[videoId]); // Log the current video reference
    ...
    if (span[videoId]) {
      ...
      const animUpdate = () => {
        if (videoRef.current[videoId] && videoRef.current[videoId].currentTime) {
      ...
      <div style={{ overflow: 'hidden', whiteSpace: 'nowrap', position: 'relative' }}>
        <div
          ...
          style={{
            ...
            width: `${100 * hightlightsSlides.length}%`,
        ...
        <div className="flex-center py-5 px-7 bg-black backdrop-blur rounded-full">
          {videoRef.current.map((_, i) => (
            <span
              ...
              className="mx-2 w-3 h-3 bg-grey rounded-full relative cursor-pointer"
            >
              <span
